...
let defaultQueries = ["education", "computer science", "biology"];
let query = "";           // current query for API calls
let page = 0;
const pageSize = 10;
let articles = [];
let isLoading = false;
...
filters.addEventListener("change", () => {
  ...
  if (!searchBar.value.trim() && activeFilters.length > 0) {
    ...
    query = activeFilters.join(" OR ");
    page = 0;
    articles = [];
    container.innerHTML = "";
    loadMore();
  } else {
    // Just filter already loaded articles
    renderArticles();
  }
});
...
async function loadMore() {
  if (isLoading) return;
  if (!query) query = "education"; // fallback if empty
  isLoading = true;
  ...
  try {
    ...
    // Background fetches
    fetchCrossRef(query, page).then(res => { articles = [...articles, ...res]; renderArticles(); }).catch(()=>{});
    fetchPubMed(query, page).then(res => { articles = [...articles, ...res]; renderArticles(); }).catch(()=>{});
...
async function fetchArxiv(query, page) {
  try {
    ...
    return entries.map(e => {
      ...
      return { title, snippet, authors, url, provider: "arXiv", category: categorize(snippet + " " + title, "arXiv") };
    });
  } catch { return []; }
}
...
// --------------------- INITIAL LOAD ---------------------
window.onload = () => {
  // Preload multiple default queries for variety
